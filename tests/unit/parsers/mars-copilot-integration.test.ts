...
describe("mars + copilot integration", () => {
  it("matches session_id, interaction_id, copilot_pid, and api_id fallback keys", async () => {
    ...
    await fs.writeFile(path.join(logDir, "process-001.log"), [
      ...
      JSON.stringify({ kind: "assistant_usage", properties: { event_id: "a1", model: "claude", interaction_id: "interaction-a", copilot_pid: "pid-a" }, metrics: { input_tokens: 1, output_tokens: 1 }, session_id: "session-key" }),
      JSON.stringify({ kind: "assistant_usage", properties: { event_id: "a2", model: "claude", interaction_id: "interaction-key", copilot_pid: "pid-b" }, metrics: { input_tokens: 1, output_tokens: 1 } }),
      JSON.stringify({ kind: "assistant_usage", properties: { event_id: "a3", model: "claude", copilot_pid: "pid-key" }, metrics: { input_tokens: 1, output_tokens: 1 } }),
  ...
  it("keeps unmatched sessions and logs debug message", async () => {
    ...
    const logDir = path.join(roots.copilot, "logs");
    await fs.mkdir(logDir, { recursive: true });
    await fs.writeFile(path.join(logDir, "process-001.log"), JSON.stringify({ kind: "assistant_usage", properties: { event_id: "evt-1", model: "claude" }, metrics: { input_tokens: 1, output_tokens: 1 } }) + "\n", "utf8");
